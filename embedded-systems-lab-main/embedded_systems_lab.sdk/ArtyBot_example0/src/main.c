...
// Check if car is blocked instead of turn right you need to stop it
int isBlocked()
{
...
// Supervisor task changes the state of my car i.e takes care of the task
...
static void prvDriveTask(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    if (xSemaphoreTake(state_mutex, 10))
    {
      ...
      {
        state = NAVIGATE; // instead of navigate stop it
      } else
    	  // xil_printf("driving forward!\r\n");
  ...
  while (1)
  {
    ...
    if (xSemaphoreTake(state_mutex, 10))
    {
      if (isBlocked() && !interrupt_occured)
    	  ...
    	  turnRight(90); // turnRight(90); // instead of navigate stop it
